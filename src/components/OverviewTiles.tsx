...
import { Users, UserCheck, TrendingUp, AlertTriangle, Database, Award } from 'lucide-react';
...
export const OverviewTiles: React.FC<OverviewTilesProps> = ({ overview }) => {
  ...
  const tiles = [
    ...
    {
      title: 'Heavy Users',
      value: overview.heavyUsers.toLocaleString(),
      icon: TrendingUp,
      color: 'bg-purple-500',
      subtitle: `${Math.round((overview.heavyUsers / overview.totalUsers) * 100)}% of total`,
      link: '/users/usage-level?value=Heavy'
    },
    {
      title: 'Inactive Users',
      value: overview.inactiveUsers.toLocaleString(),
      icon: AlertTriangle,
      color: 'bg-red-500',
    ...
    },
    {
      ...
      icon: Database,
      color: 'bg-indigo-500',
      subtitle: `${overview.topObjects[0]?.usage.toLocaleString()} touches`,
      link: `/object/${overview.topObjects[0]?.name || 'Account'}`
    },
    {
      title: 'License Efficiency',
      value: `${Math.round((overview.licenseUtilization.full.used / overview.licenseUtilization.full.total) * 100)}%`,
      icon: Award,
      color: 'bg-orange-500',
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6 mb-8">
      {tiles.map((tile, index) => (
        ...
        >
          ...
          <div>
            <h3 className="text-sm font-medium text-gray-600 mb-1">{tile.title}</h3>
            <p className="text-2xl font-bold text-gray-900 mb-1">{tile.value}</p>
            {tile.subtitle && (
